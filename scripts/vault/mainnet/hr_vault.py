...
def deploy_vault_proxy(
    ...
    if not account:
        account = get_account()
    vault = Contract.from_abi("HRVault", vault_impl_address, HRVault.abi)
    vault_encoded_initializer_function = encode_function_data(
        vault.initialize,
    )
    vault_proxy = HRVaultProxy.deploy(
        vault_impl_address,
...
def deploy_vault_proxy_admin(account: Optional[Account] = None) -> str:
    if not account:
        account = get_account()
    # Deploy proxy admin
    proxy_admin = HRVaultProxyAdmin.deploy({"from": account})
...
def deploy_vault_implementation(account: Optional[Account] = None) -> str:
    if not account:
        account = get_account()
    vault_implementation = HRVault.deploy({"from": account})
...
def verify_vault_proxy_admin(proxy_admin_address: str):
    contract = HRVaultProxyAdmin.at(proxy_admin_address)
    HRVaultProxyAdmin.publish_source(contract)


def verify_vault_proxy(proxy_address: str):
    contract = HRVaultProxy.at(proxy_address)
    HRVaultProxy.publish_source(contract)


def verify_vault(vault_address: str):
    contract = HRVault.at(vault_address)
    HRVault.publish_source(contract)
